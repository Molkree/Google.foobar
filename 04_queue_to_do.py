...
def f(n):
    # sequential xor's cycle through n, 1, n+1, 0
    cycle = [n, 1, n + 1, 0]
    return cycle[n % 4]


def xor_range(start, end_inclusive):
    # start ^ start+1 ^ ... ^ end-1 ^ end =
    # (1 ^ ... ^ end) ^ (1 ^ ... ^ start-1) =
    # 1 ^ 1 ^ ... start-1 ^ start-1 ^ ... start ^ ... ^ end
    return f(end_inclusive) ^ f(start - 1)
...
def solution(start, length):
    ...
    new_length = length
    while new_length > 0:
        result ^= xor_range(start, start + new_length - 1)
        new_length -= 1
        start += length
    return result


assert 2 == solution(0, 3)

assert 14 == solution(17, 4)

assert 23 == solution(20, 2)

assert 6 == solution(30, 3)

assert 14 == solution(0, 5)

assert 2000000001 == solution(1999999998, 2)

assert 0 == solution(0, 1)

assert 100 == solution(100, 1)
